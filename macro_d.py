...
def train_filter(df):
    ...
    first_row = df.iloc[0]
    x = np.array([[1], [1], [0], [0.5], [0.01], [first_row['s']], [0.01], [0.5], [0.01], [0], [0.001]])
    ekf = MacroEKF()
    ekf.x = x
    ekf.compute_f()
    time = []
    ...
    for index, row in df.iterrows():
        ...
        z = [row['k'], row['s'], row['n'], row['Num Y_L']]
        # actual_k.append(z[0])
    ...
    d_df.to_csv("d.csv")
    print("actual d : ", avg(actual_d))
    print("predicted d : ", avg(predicted_d))
    plt.subplot(3, 1, 1)
